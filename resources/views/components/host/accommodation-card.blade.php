...
@php
    ...
    $bookingsCount = $acc->bookings_count ?? $acc->total_bookings_count ?? 0;
@endphp

<div {{ $attributes->class(['card shadow-sm h-100 host-acc-card']) }}>
    ...
    <div class="host-acc-card__content">
        ...
        <div class="row text-center g-2 mt-1">
            ...
            <div class="col-6">
                @if($actions === 'full' && $hostUser->hostCan('bookings.list', 'read'))
                <a wire:navigate href="{{ route('host.bookings.index', ['accommodation_id'=> $acc->id]) }}" class="text-decoration-none">
                    <div class="fw-bold text-success">{{ $bookingsCount }}</div>
        ...
        <div class="host-acc-card__actions">
            @if($actions === 'quick')
                @if($hostUser->hostCan('accommodations.manual-service-sale', 'write'))
                <a wire:navigate href="{{ route('host.accommodations.manual-service-sale', $acc) }}" class="btn btn-sm btn-outline-success flex-fill">
                    <i class="bi bi-bag-plus me-1"></i>فروش خدمات
                </a>
                @endif
                ...
                <a wire:navigate href="{{ route('host.accommodations.manual-booking', $acc) }}" class="btn btn-sm btn-success flex-fill">
                    <i class="bi bi-plus-circle me-1"></i>رزرو
                </a>
                @endif
            @else
