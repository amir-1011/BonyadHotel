<div id="supportiveReportRoot" data-report-id="{{ $componentId }}" wire:key="sup-report-{{ $filterKey }}-{{ $period }}-{{ $jalaliYear }}-{{ $jalaliMonth }}">
...
@php
    ...
    $discountDisplay = $compactToman((int) $kpis['discount']);
    $avgDiscountDisplay = $compactToman((int) $kpis['avg_discount']);
...
<div class="ta-card med-kpi mb-3">
    <div class="med-kpi__grid">
        ...
        <article class="med-kpi__cell">
            <span class="med-kpi__icon med-kpi__icon--info" aria-hidden="true"><i class="bi bi-percent"></i></span>
            <div class="med-kpi__copy">
                <div class="med-kpi__label">جمع تخفیف حمایتی</div>
                <div class="med-kpi__value-row">
                    <strong class="med-kpi__value">{{ $discountDisplay['num'] }}</strong>
                    <span class="med-kpi__unit">{{ $discountDisplay['unit'] }}</span>
                </div>
                <div class="med-kpi__meta">میانگین {{ $avgDiscountDisplay['num'] }} {{ $avgDiscountDisplay['unit'] }}</div>
            </div>
        </article>
        ...
        <article class="med-kpi__cell">
            ...
            <div class="med-kpi__copy">
                <div class="med-kpi__label">مهمانان</div>
                <div class="med-kpi__value-row">
                    <strong class="med-kpi__value">{{ $fa($kpis['guests']) }}</strong>
                    <span class="med-kpi__unit">نفر</span>
                </div>
                <div class="med-kpi__meta">رزروهای تأییدشده</div>
            </div>
        </article>
        ...
        <article class="med-kpi__cell">
            ...
            <div class="med-kpi__copy">
                <div class="med-kpi__label">برنامه / اردوی حمایتی</div>
                <div class="med-kpi__value-row">
                    <strong class="med-kpi__value">{{ $fa($kpis['programs']) }}</strong>
                    <span class="med-kpi__unit">برنامه</span>
                </div>
                <div class="med-kpi__meta">فقط برنامه‌های حمایتی همین بازه</div>
            </div>
        </article>

        @php
            $groupIcons = [
                \App\Support\SupportiveServicesReportGroups::KEY_SUPPORTIVE_PROGRAM => ['class' => 'med-kpi__icon--rose', 'icon' => 'bi-heart-fill'],
                'veteran_70_spouses' => ['class' => 'med-kpi__icon--primary', 'icon' => 'bi-award-fill'],
                'veteran_50_69_dependents' => ['class' => 'med-kpi__icon--teal', 'icon' => 'bi-people-fill'],
                'veteran_25_49_dependents' => ['class' => 'med-kpi__icon--amber', 'icon' => 'bi-person-hearts'],
                'martyr_children' => ['class' => 'med-kpi__icon--warning', 'icon' => 'bi-star-fill'],
                'martyr_parents_dependents' => ['class' => 'med-kpi__icon--success', 'icon' => 'bi-house-heart-fill'],
                'martyr_spouse_dependents' => ['class' => 'med-kpi__icon--secondary', 'icon' => 'bi-star-fill'],
                'freed_prisoner_dependents' => ['class' => 'med-kpi__icon--violet', 'icon' => 'bi-flag-fill'],
                'foundation_staff_retirees' => ['class' => 'med-kpi__icon--info', 'icon' => 'bi-briefcase-fill'],
            ];
        @endphp
        @foreach(($kpis['shared_groups'] ?? []) as $group)
            @php
                $visual = $groupIcons[$group['key']] ?? ['class' => 'med-kpi__icon--violet', 'icon' => 'bi-heart-fill'];
                $groupDiscount = $compactToman((int) $group['discount']);
            @endphp
            <article class="med-kpi__cell">
                ...
                <div class="med-kpi__copy">
                    ...
                    <div class="med-kpi__meta">{{ $fa($group['nights']) }} شب · {{ $groupDiscount['num'] }} {{ $groupDiscount['unit'] }}</div>
                    @if(($group['guests'] ?? 0) > 0)
                    <div class="med-kpi__meta">{{ $fa($group['guests']) }} مهمان</div>
                    @endif
                </div>
            </article>
        @endforeach
...
<script type="application/json" id="supportive-report-map-payload" wire:ignore>{!! json_encode($mapPayload, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) !!}</script>

<div class="ta-card mb-4 med-map-card">
    <div class="ta-card__head">
        <div>
            <h2 class="ta-card__title">پراکندگی رزروهای خدمات حمایتی در ایران</h2>
            <div class="ta-card__sub">تعداد رزروهای تأییدشده خدمات حمایتی و ایثارگری بر اساس استان و شهر</div>
        </div>
    ...
    <div class="ta-card__body">
        <div class="row g-3 align-items-stretch">
            <div class="col-12 col-lg-7">
                <div id="supportiveIranMap" class="med-iran-map" wire:ignore></div>
            </div>
            <div class="col-12 col-lg-5">
                ...
                <div class="d-flex flex-column gap-2" id="supportiveCityList">
                    ...
                    @empty
                        <div class="text-muted text-center py-5">در این بازه رزرو تأییدشده خدمات حمایتی ثبت نشده است</div>
                    @endforelse
                </div>
            </div>
        </div>
...
<div class="ta-card mb-4">
    <div class="ta-card__head">
        <div>
            ...
            <div class="ta-card__sub">جزئیات رزرو تأییدشده خدمات حمایتی در هر استان — کلیک روی ردیف، رزروهای همان استان را نشان می‌دهد</div>
        </div>
    </div>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr role="button" wire:click="openProvince({{ (int) $row->province_id }})" style="cursor:pointer">
                        ...
                        <td class="fw-semibold">{{ $fa($row->discount) }} ریال</td>
                    </tr>
                    @empty
                    <tr><td colspan="9" class="text-center text-muted py-4">داده‌ای برای این فیلتر نیست</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
<div class="row g-3 mb-3">
    <div class="col-12 col-lg-6">
        <div class="ta-card h-100 mb-0">
            <div class="ta-card__head">
                <div>
                    <h2 class="ta-card__title">گزارش شهرها</h2>
                    <div class="ta-card__sub">رزرو تأییدشده خدمات حمایتی به تفکیک شهر — کلیک، مودال رزروهای همان شهر را باز می‌کند</div>
                </div>
            </div>
            <div class="ta-card__body p-0">
                <div class="table-responsive">
                    <table class="table table-hover table-sm align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr role="button" wire:click="openCity({{ (int) $c->city_id }})" style="cursor:pointer">
                                ...
                                <td>{{ $fa($c->bookings) }}</td>
                                <td>{{ $fa($c->nights) }}</td>
                                <td class="fw-semibold">{{ $fa($c->discount) }}</td>
                            </tr>
                            @empty
    ...
    <div class="col-12 col-lg-6">
        <div class="ta-card h-100 mb-0">
            <div class="ta-card__head">
                <h2 class="ta-card__title mb-0"><i class="bi bi-houses me-1"></i>اقامتگاه</h2>
            </div>
            <div class="ta-card__body p-0">
                <div class="table-responsive">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title d-flex align-items-center gap-2">
                    ...
                    رزروهای خدمات حمایتی — {{ $modalTitle }}
                </h5>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="small">{{ $reportService->reportGroupLabel($booking) }}</td>
                                <td class="small">{{ $booking->program?->title ?: '—' }}</td>
                                <td class="small">{{ $booking->program?->employer?->name ?? $booking->employer?->name ?? '—' }}</td>
                                <td class="small fw-semibold">{{ \App\Support\PdfPersian::toPersianDigits(number_format($booking->discount_amount)) }}</td>
                                <td><span class="badge bg-{{ $booking->statusColor() }}">{{ $booking->statusLabel() }}</span></td>
                            </tr>
                            @empty
                            <tr><td colspan="{{ $selectedProvinceId ? 11 : 10 }}" class="text-center text-muted py-4">رزروی برای این محدوده در بازه انتخابی نیست</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
...
<style>
    ...
    #supportiveIranMap .leaflet-container {
        background: #f9fafb;
        cursor: default;
    }
    #supportiveIranMap .leaflet-control-zoom,
    #supportiveIranMap .leaflet-control-attribution {
        display: none !important;
    }
    #supportiveIranMap .leaflet-grab,
    #supportiveIranMap .leaflet-dragging .leaflet-grab {
        cursor: default;
    }
    #supportiveCityList .city-row:hover {
...
(function () {
    ...
    const ns = window.__taSupportiveReportMap = window.__taSupportiveReportMap || {};
    const faNum = n => new Intl.NumberFormat('fa-IR').format(n || 0);

    function readPayload() {
        const el = document.getElementById('supportive-report-map-payload');
    ...
    function reportComponent() {
        const id = document.getElementById('supportiveReportRoot')?.dataset.reportId;
    ...
    function destroyMap() {
        ...
        const el = document.getElementById('supportiveIranMap');
    ...
    function initMap() {
        const mapEl = document.getElementById('supportiveIranMap');
            ...
            .then(geo => {
                ...
                geoLayer = L.geoJSON(geo, {
                    ...
                    onEachFeature: (f, lyr) => {
                        ...
                        lyr.bindTooltip(
                            `<div style="font-family:Vazirmatn,sans-serif;font-size:12px;text-align:right"><b>${name}</b><br>${faNum(v)} رزرو خدمات حمایتی</div>`,
    ...
    function boot() {
        if (!document.getElementById('supportiveIranMap')) return;
    ...
    if (!ns.bound) {
        ...
        document.addEventListener('supportive-report-map-refresh', boot);
        if (window.Livewire) {
            Livewire.on('supportive-report-map-refresh', boot);
        }
    }
