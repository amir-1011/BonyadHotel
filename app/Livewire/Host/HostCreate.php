...
#[Layout('layouts.host', ['title' => 'افزودن میزبان', 'pageTitle' => 'افزودن میزبان'])]
class HostCreate extends Component
{
    ...
    {
        $this->authorizeHostCan('users.create-host', 'write');
    ...
    public function save(): void
    {
        ...
        $invalidAccommodation = collect($this->orderedSelectedAccommodationIds())
            ->first(fn (int $id) => !in_array($id, $managedIds, true));

        if ($invalidAccommodation) {
            $this->addError('selectedAccommodationIds', 'فقط اقامتگاه‌های تحت مدیریت شما قابل انتخاب هستند.');
        ...
        if (!filled($user->personnel_code)) {
            $this->addError('accountingProvinceId', 'کد پرسنلی تخصیص داده نشد. کد حسابداری استان را بررسی کنید.');
        ...
        $this->redirectRoute('host.users.edit', $user, navigate: true);
    }
    ...
    public function render()
    {
        ...
        return view('host.users.create-host', [
